...
from hybrik_utils import get_affine_transform
...
class Simple3DPoseBaseSMPL(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d):
        ...
        self.norm_type = 'softmax' #kwargs['POST']['NORM_TYPE']
        self.depth_dim = 64 #kwargs['EXTRA']['DEPTH_DIM']
        self.height_dim = 64 #kwargs['HEATMAP_SIZE'][0]
        self.width_dim = 64 #kwargs['HEATMAP_SIZE'][1]
        self.smpl_dtype = torch.float32

        backbone = ResNet

        self.preact = backbone("resnet101")

        kwargs = {'NUM_LAYERS' : 101}
        ...
        h36m_jregressor = np.load('./logs/run_time_utils/J_regressor_h36m.npy')
        self.smpl = SMPL_layer(
            './logs/run_time_utils/basicModel_neutral_lbs_10_207_0_v1.0.0.pkl',
        ...
        # mean shape
        init_shape = np.load('./logs/run_time_utils/h36m_mean_beta.npy')
...
if __name__ == "__main__":
    ...
    inp_w, inp_h = 256, 256
    trans_inv = get_affine_transform(center, scale, 0, [inp_w, inp_h], inv=True).astype(np.float32)
    intrinsic_param = np.zeros((3, 3)).astype(np.float32)
    joint_root = np.zeros((3)).astype(np.float32)
    bbox_3d_shape = 0
    depth_factor = np.array([bbox_3d_shape[2]]).astype(np.float32) if bbox_3d_shape else np.zeros((2000)).astype(np.float32)
